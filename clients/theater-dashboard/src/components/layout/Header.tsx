  ...
  return (
    <header className={style.container}>
      ...
      <nav className={style.navs}>
        ...
        >
          {theme === "white" ? (
            <DarkThemeIcon strokeWidth={1.5} size={23} />
          ) : (
            <WhiteThemeIcon strokeWidth={1.5} size={23} />
          )}
        </div>
      ...
      <aside className={showSidebar ? style["active-sidebar"] : style.sidebar}>
        {sidebarNavs.map((sidebarnav, index) => (
          <NavLink
            ...
            key={index}
            className={
              isLinkActive(pathname, sidebarnav.route)
                ? style["sidebar-active-nav"]
                : style["sidebar-nav"]
            }
            onClick={() => setShowSidebar(false)}
          >
        ...
        <NavLink className={style["sidebar-nav"]} to={envVariables.APP_URL}>
          Home
        </NavLink>
      </aside>
    </header>
