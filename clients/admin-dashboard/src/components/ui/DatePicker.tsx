...
const DatePicker = ({
  ...
  const memoShowTimePicker = useMemo(() => showTimePicker, [showTimePicker]);
  const callbackClickOnTime = useCallback(
  ...
  const handleSetCalendarDates = useCallback(() => {
  ...
  const handleGetAvailableTimes = useCallback(() => {
    ...
    if (
      ...
      // Pushing 8:00 AM to 10:30 PM times
      for (let i = 8; i <= 22; i++) {
        availableTimes.push({ hour: i, minute: 0 });
        availableTimes.push({ hour: i, minute: 30 });
      }
    ...
    if (callbackClickOnTime) {
      callbackClickOnTime(availableTimes[0]);
    }
  }, [day, month, year, callbackClickOnTime]);
  ...
  useEffect(() => {
    ...
    if (memoShowTimePicker) {
      (() => {
        handleGetAvailableTimes();
      })();
    }
  }, [month, year, day, memoShowTimePicker, handleSetCalendarDates, handleGetAvailableTimes]);

  return (
    <div className={style.container}>
      <div className={style["top-bar"]}>
        <div className={style["icon-container"]} onClick={handleDecrementMonth}>
          <LeftArrowIcon
            size={20}
            strokeWidth={1.6}
        ...
        <div className={style["icon-container"]} onClick={handleIncrementMonth}>
          <RightArrowIcon
            size={20}
            strokeWidth={1.6}
            className={style["icon"]}
      ...
      <div className={style["weeks-container"]}>
        {weeks.map((week, index) => (
          <span key={index} className={style.week}>
            {week}
          </span>
        ))}
      </div>
