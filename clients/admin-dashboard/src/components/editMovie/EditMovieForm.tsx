...
const EditMovieForm = () => {
  ...
  const submitForm = async (data: Inputs) => {
    ...
    if (poster) {
      posterResult = await updateMovieImageApi(
        "poster",
        movieId as string,
        poster,
      );
    } else if (banner) {
      bannerResult = await updateMovieImageApi(
        "banner",
        movieId as string,
        banner,
      );
  ...
  }, [movieId, handleFetchMovieDetails]);
  ...
  return movieDetails ? (
    <form onSubmit={handleSubmit(submitForm)} className={style.container}>
      ...
      <div className={style.poster}>
        {movieDetails?.poster && showPoster ? (
          <div className={style["image-container"]}>
            ...
            >
              <CloseIcon className={style.icon} size={22} />
            </div>
          </div>
        ) : (
          <ImagePicker
      ...
      <div className={style["form-input"]}>
        ...
        />
        <ShowTrailer trailerUrl={movieDetails.movie_trailer} />
      </div>
      {/* Movie Categories */}
      <div className={style["checkbox-container"]}>
        <FormLabel title="Movie Categories" />
        <CheckBoxList
