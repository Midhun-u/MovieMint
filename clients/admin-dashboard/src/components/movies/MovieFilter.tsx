import { Activity, createContext, useContext, useEffect, useState, type Dispatch, type JSX, type SetStateAction } from 'react'
...
const MovieFilter = ({ setSelectedCategories, setSelectedFormats, setSelectedLanguage }: MovieFilterProps) => {

    const [showFilterScreen, setShowFilterScreen] = useState<boolean>(false)
    const [ScreenDetails, setScreenDetails] = useState<{ title: string, Screen: () => JSX.Element | null }>({
        title: "",
        Screen: () => null
    })
    ...
    return (
        <>
            <div onClick={() => setShowFilterScreen(true)} className={style.container}>
                <FilterIcon
                ...
                <div className={style['filter-screen-container']}>
                    ...
                    <div className={style['filter-screen']}>
                        <div className={style['top-bar']}>
                            <span>Filter By</span>
                            <div onClick={() => setShowFilterScreen(false)} className={style['icon-container']}>
                                <CloseIcon
                                    className={style.icon}
                                    strokeWidth={1.7}
                                    size={20}
                        ...
                        <div className={style['filter-options']}>
                            <div className={showScreen ? style['hide-options'] : style.options}>
                                <li
                                    className={ScreenDetails.title === "genre" ? style.active : ""}
                                    onClick={() => handleSetScreen("genre", MovieGenre)}
                                >
                                    Genre
                                </li>
                                <li
                                    className={ScreenDetails.title === "languages" ? style.active : ""}
                                    onClick={() => handleSetScreen("languages", MovieLanguage)}
                                >
                                    Languages
                                </li>
                                <li
                                    className={ScreenDetails.title === "formats" ? style.active : ""}
                        ...
                        <div className={style['button-container']}>
                            <Button className={showScreen ? style['show-back-button'] : style['back-button']} onClick={() => setShowScreen(false)}>
                                <BackIcon
                                    size={20}
                                    strokeWidth={1.5}
                                />
